import os
import io                                                           # PARALEL IÇIN YAPILDI
import random
from concurrent.futures import ProcessPoolExecutor                  # PARALEL IÇIN YAPILDI
from contextlib import redirect_stdout                              # PARALEL IÇIN YAPILDI
...
from algorithm.genetic_paralel import ParallelGeneticDomainSolver  # PARALEL IÇIN YAPILDI
...
#   takip edebiliriz.
# -----------------------------------------------------------------------
# PARALEL IÇIN YAPILDI: TrackingGeneticSolver artık ParallelGeneticDomainSolver'dan türüyor.
# Fitness hesaplaması worker process'lerde yapılır; _evaluated_chroms ParallelGeneticDomainSolver.run()
# içindeki "hasattr(self, '_evaluated_chroms')" kontrolüyle ana process'te doldurulur.
class TrackingGeneticSolver(ParallelGeneticDomainSolver):
    ...
    def calculate_fitness_v2(self, chromosome):
        # PARALEL IÇIN YAPILDI: paralel modda bu metot worker'larda çalışır,
        # _evaluated_chroms güncellemesi run() içinde yapılır.
...
INTRA_NODE_COUNTS = [9]  # her değer için bağımsız çalışma
...
# PARALEL IÇIN YAPILDI: yöntem düzeyinde paralel çalışma için maksimum worker process sayısı.
# None → os.cpu_count() kullanılır. Sunucudaki çekirdek sayısına göre ayarlayın.
...
# -----------------------------------------------------------------------
# PARALEL IÇIN YAPILDI: modül düzeyindeki load_inter_network() main() içine taşındı.
# Windows'ta ProcessPoolExecutor spawn modunda her worker modülü yeniden import eder;
# modül düzeyinde dosya okuma her worker'da gereksiz yere çalışırdı.
# -----------------------------------------------------------------------


# PARALEL IÇIN YAPILDI: yöntem düzeyinde paralel worker fonksiyonları.
# ProcessPoolExecutor pickle kullanır; fonksiyonların modül düzeyinde tanımlı olması gerekir.
def _greedy_worker(args):
    # PARALEL IÇIN YAPILDI: tek greedy yöntemi ayrı process'te çalışır, çıktı tamponlanır
    label, solver_class, cent_method, allTr, er, interNet, intraTop, vr = args
    buf = io.StringIO()
    with redirect_stdout(buf):
        chrom, fitness, solver = run_greedy_method(
            label, solver_class, cent_method, allTr, er, interNet, intraTop, vr,
        )
    return label, chrom, fitness, solver, buf.getvalue()


def _ga_worker(args):
    # PARALEL IÇIN YAPILDI: tek GA yöntemi ayrı process'te çalışır, çıktı tamponlanır
    label, mode, temp, allTr, er, interNet, intraTop, vr = args
    buf = io.StringIO()
    with redirect_stdout(buf):
        chrom, fitness, solver = run_method(
            label, mode, temp, allTr, er, interNet, intraTop, vr,
        )
    return label, chrom, fitness, solver, buf.getvalue()
...
# -----------------------------------------------------------------------
def main():
    # PARALEL IÇIN YAPILDI: modül düzeyinden buraya taşındı
    ...
    for n_nodes in INTRA_NODE_COUNTS:
        ...
        for vr_idx, (virtualRequests, vr_fname) in enumerate(zip(virtual_requests_list, vr_filenames)):
            ...
            # PARALEL IÇIN YAPILDI: greedy ve GA yöntemleri eşzamanlı çalıştırılıyor.
            # Her yöntem ayrı bir process'te çalışır; stdout tamponlanır ve sırayla basılır.
            args_greedy = [
                (label, cls, cent,
                 allTransaction, edgeRouter, interNetwork, intraTopologies, virtualRequests)
                for label, _, cls, cent in GREEDY_METHODS
            ]
            args_ga = [
                (label, mode, temp,
                 allTransaction, edgeRouter, interNetwork, intraTopologies, virtualRequests)
            ...
            n_methods = len(args_greedy) + len(args_ga)
            # PARALEL IÇIN YAPILDI: tüm işler aynı anda gönderiliyor; sonuçlar sıra korunarak toplanıyor
            with ProcessPoolExecutor(
                max_workers=min(MAX_WORKERS or os.cpu_count(), n_methods)
            ) as exe:
                greedy_futures = [exe.submit(_greedy_worker, a) for a in args_greedy]
                ga_futures     = [exe.submit(_ga_worker,     a) for a in args_ga]

                # PARALEL IÇIN YAPILDI: gönderme sırası korunarak sonuçlar alınıyor
                greedy_results = [f.result() for f in greedy_futures]
                ga_results     = [f.result() for f in ga_futures]

            # PARALEL IÇIN YAPILDI: tamponlanan çıktılar gönderme sırasıyla basılıyor
            for label, chrom, fitness, solver, output in greedy_results:
                print(output, end="")
                greedy_sonuclar.append((label, chrom, fitness))
                greedy_solvers.append(solver)

            for label, chrom, fitness, solver, output in ga_results:
                print(output, end="")
...
# PARALEL IÇIN YAPILDI: Windows'ta ProcessPoolExecutor spawn modu modülü her worker'da yeniden import eder.
# if __name__ == '__main__' koruması olmadan main() her worker'da da çalışırdı.
