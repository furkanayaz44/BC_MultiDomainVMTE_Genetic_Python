import random
import math
import os                                                    # PARALEL IÇIN YAPILDI
...
# PARALEL IÇIN YAPILDI: modül düzeyinde worker fonksiyonları
# ProcessPoolExecutor pickle gerektirir; sınıf metotları pickle edilemez.
# _init_worker solver'ı her worker process'e bir kez gönderir (modül global'ine atar).
# _fitness_worker her kromozom için çağrılır — solver verisi zaten worker bellekte.
_worker_solver = None


def _init_worker(solver):
    # PARALEL IÇIN YAPILDI: worker process başlarken bir kez çalışır
    global _worker_solver
    _worker_solver = solver


def _fitness_worker(chrom):
    # PARALEL IÇIN YAPILDI: worker process'te fitness hesaplanır
    return _worker_solver.calculate_fitness_v2(chrom)
...
class ParallelGeneticDomainSolver(GeneticDomainSolver):
    """
    PARALEL IÇIN YAPILDI:
    GeneticDomainSolver'ın paralel fitness hesaplamalı versiyonu.
    Her nesilde tüm population fitness değerlendirmesi ProcessPoolExecutor ile
    eşzamanlı yapılır. Geri kalan her şey (crossover, mutasyon, seçim) aynıdır.

    n_workers=None → os.cpu_count() kadar process kullanılır.
    """

    def __init__(self, *args, n_workers=None, **kwargs):
        super().__init__(*args, **kwargs)
        # PARALEL IÇIN YAPILDI: worker process sayısı; None ise CPU çekirdeği sayısı
    ...
    def run(self, population_size=20, generations=100, mutation_rate=0.1, seed=None):
        ...
        # PARALEL IÇIN YAPILDI: ProcessPoolExecutor tüm nesiller için bir kez açılır.
        # initializer ile solver her worker'a bir kez picklelanır; tekrar gönderilmez.
        with ProcessPoolExecutor(
            ...
            for gen in range(1, generations + 1):
                ...
                # PARALEL IÇIN YAPILDI: tüm population fitness değerlendirmesi eşzamanlı
                fits = list(exe.map(_fitness_worker, population))

                scored_pop = []
                for chrom, fit in zip(population, fits):
                    # PARALEL IÇIN YAPILDI: Q-update ana process'te sırayla yapılıyor
                    # (Q-tablosu paylaşılan state; worker'lara gönderilemez)
                    self._ql_update(chrom, fit)
                    scored_pop.append((chrom, fit))
                    if fit < best_fitness:
                        best_fitness  = fit
                        best_solution = chrom
                    # PARALEL IÇIN YAPILDI: TrackingGeneticSolver uyumluluğu için
                    # _evaluated_chroms mevcutsa ana process'te doldurulur
                    if hasattr(self, '_evaluated_chroms'):
                        self._evaluated_chroms.append(tuple(chrom))
                ...
                if gen % 10 == 0 or gen == 1:
                    eps_str = (
                        f"  epsilon={self.ql_epsilon:.3f}"
                        if self.selection_mode == 'qlearning' else ""
                    )
                    print(f"Jenerasyon {gen}: En Iyi Fitness (Maliyet) = {best_fitness}{eps_str}")
